...
select puesto, nombre, apellido1,apellido2 from empleado where puesto like 'Director General';
...
select *, year(fecha_pago) from pago;

select* from pago where year(fecha_pago)=2008;

select *, date_format(fecha_pago, "%y") from pago;
...
/*select * from pedido where adddate(fecha_entrega, inverval 2 day) <= fecha_esperada;*/
...
/* adddate funciona con dias, meses y años*/

select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido WHERE DATEDIFF(fecha_entrega,fecha_esperada ) >= 2;   /*solo funciona con dias*/


/*datediff hace la diferencia*/
...
SELECT cli.nombre_cliente, empl.nombre, empl.apellido1, empl.apellido2, empl.puesto
FROM cliente AS cli
...
SELECT cli.nombre_cliente, empl.nombre, empl.apellido1, empl.apellido2, empl.puesto, p.forma_pago
FROM cliente AS cli
INNER JOIN empleado AS empl
ON cli.codigo_empleado_rep_ventas = empl.codigo_empleado
...
SELECT distinct cli.codigo_cliente, cli.nombre_cliente, concat(empl.nombre, ' ', empl.apellido1, ' ', empl.apellido2) AS 'Vendedor', empl.puesto, p.forma_pago
FROM cliente AS cli
INNER JOIN empleado AS empl
ON cli.codigo_empleado_rep_ventas = empl.codigo_empleado
INNER JOIN pago AS p
...
INNER JOIN empleado AS empl
ON cli.codigo_empleado_rep_ventas = empl.codigo_empleado
WHERE cli.codigo_cliente NOT IN (SELECT pago.codigo_cliente FROM pago)
ORDER BY cli.codigo_cliente;
...
SELECT DISTINCT c.nombre_cliente, CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2) AS 'Vendedor', 
e.puesto, o.ciudad
FROM cliente as c
...
SELECT DISTINCT c.codigo_cliente, c.nombre_cliente, CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2) AS 'Vendedor', 
e.puesto, o.ciudad
FROM cliente as c
INNER JOIN empleado as e
ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
SELECT DISTINCT c.codigo_cliente, c.ciudad, c.nombre_cliente, 
CONCAT(o.linea_direccion1,' ', o.linea_direccion2) as direccion_oficiona
FROM cliente as c
INNER JOIN empleado as e
ON c.codigo_empleado_rep_ventas = e.codigo_empleado
INNER JOIN oficina as o
...
SELECT DISTINCT c.codigo_cliente, c.ciudad, c.nombre_cliente, o.ciudad,
CONCAT(o.linea_direccion1,' ', o.linea_direccion2) as direccion_oficiona
FROM cliente as c
INNER JOIN empleado as e
ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
SELECT COUNT(codigo_empleado) as 'Cantidad empleados' FROM empleado;
...
SELECT ROUND(AVG(total)) as 'Pago Medio' FROM pago;
...
select count(codigo_cliente) as 'Total Clientes Madrid' from cliente where ciudad like 'Madrid';
